    ...
    @else
        <div class="card card-dark">
            <div class="card-header">
                <h3 class="card-title">Form Buletin</h3>
            </div>

            @if (isset($dataBuletin))
                <form id="quickForm" action="{{ app(CustomClass::class)->rootApp() }}/form/buletin/edit" method="post"
                    enctype="multipart/form-data">
                    <input type="hidden" name="lampiran_lama" value="{{ $dataBuletin->lampiran }}">
                    <input type="hidden" name="id" value="{{ $dataBuletin->id }}">
                @else
                    <form id="quickForm" action="{{ app(CustomClass::class)->rootApp() }}/form/buletin/submit"
            ...
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control @if ($errors->has('edisi')) is-invalid @endif"
                            name="edisi" placeholder="Edisi"
                            value="{{ old('edisi', isset($dataBuletin) ? $dataBuletin->edisi : '') }}" required>
                    </div>
                </div>
                <div class="form-group row">
